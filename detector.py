...
def transcribe_and_detect(audio_path, non_vocal_csv_path="non-vocal sound.csv"):
    ...
    pd.DataFrame(all_sentences).to_csv("results/all_sentences_with_non_vocal_word_timestamp.csv", index=False)
    pd.DataFrame(detected_only).to_csv("results/non_vocal_detected_words.csv", index=False)
    with open("results/non_vocal_detected_words.json", "w") as f:
    ...
    return {
        ...
        "detected_non_vocal_sounds": detected_only
    }
